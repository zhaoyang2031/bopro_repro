...
def _bo(target, gen_model, repr_model, bbox_model, prompt_cls, task=None, seed=17, device='cuda', save_results=True,
    ...
    sampled_x_repr = get_representations(sampled_x,
                                         ...
                                         pooling=args.repr_llm_pooling,
                                         prompt=args.repr_prompt,
                                         device=device).view(len(sampled_x), -1)
    ...
    sampled_y = sampled_y.view(len(sampled_x))
    valid_samples = valid_samples.view(len(sampled_x))
        ...
        with ThreadPoolExecutor(max_workers=args.vec2text_n_parallel) as executor:
            ...
            futures = [executor.submit(
                lambda: decode_repr(proposed_x_repr=proposed_x_reprs[__i // (_n_parallel_calls // opt_batch_size)],
                                    ...
                                    counters=_counters_for_decoding[__i]))
