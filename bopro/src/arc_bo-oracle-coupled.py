...
from utils.generation import HF_PIPELINE_MODELS, BEDROCK_CLAUDE_MODELS, SYS_PROMPT, \
    ...
    BEDROCK_COHERE_MODELS, invoke_bedrock_embeddings, HF_SFORMER_MODELS, fix_candidate, get_openai_client, OPENAI_MODELS
...
def get_ladder_data(sampled_y_preds, expected_pred_dim, dtype, device):
    sampled_y_preds_flat = []
    for s in sampled_y_preds:
        try:
            _flat_preds = [_si for _s in s for _si in _s[0]]
            assert len(_flat_preds) == expected_pred_dim
        except:
            _flat_preds = [-1] * expected_pred_dim
        sampled_y_preds_flat.append(_flat_preds)
    sampled_y_preds = torch.tensor(sampled_y_preds_flat, dtype=dtype, device=device)

    return sampled_y_preds
...
def _bo(target, gen_model, repr_model, bbox_model, prompt_cls, task=None,
    ...
    if True:  # sampled_y is None:
        sampled_y, valid_samples, sampled_y_preds, _ = get_bbox_values(x=sampled_x, task=task, bbox_model=bbox_model,
                                                                       prompt_cls=prompt_cls,
                                                                       mode="train", llm_log=LLM_LOGS[seed]["bbox"],
                                                                       bbox_log=bbox_logs,
                                                                       n_tasks=args.bbox_n_train, device=device,
                                                                       return_predictions=True, )
        expected_pred_dim = len([ti for t in task['train'] for ti in t[1][0]])
        sampled_y_preds = get_ladder_data(sampled_y_preds, expected_pred_dim, dtype=sampled_y.dtype, device=device)
    else:
    ...
    if args.surr_skip_invalid_candidates:
        ...
        sampled_y_preds = sampled_y_preds[valid_samples]
        # if requires_bo:
    ...
    if requires_bo:
        add_kwargs = {
            ...
            **{k: v for k, v in {"matern_nu": args.kernel_matern_nu,
                                 ...
                                 "bnn_activation": args.bnn_activation,
                                 "bnn_hidden_dim": args.bnn_hidden_dim,
                                 "ladder_kernel": True}.items() if v is not None}
        }
        ...
        add_kwargs["ladder_kernel_data"] = sampled_y_preds
    ...
    # Oracle selection candidates
    all_cands, all_reprs, best_oracle_rank_sim_iter, oracle_fix_all_cands_count = None, None, None, 0
    if not opt_found:
        with open(args.all_cands_path, "r") as fh:
            raw_all_cands = json.load(fh)
        _all_cands_scores = [(a['docstring'], a['code'], a['score']) for a in raw_all_cands[target.split(".")[0]]]
        if args.oracle_only_valid:
            _all_cands_scores = [_cand for _cand in _all_cands_scores if _cand[2] != INV_SCORE]
        all_cands = [(a[0], a[1]) for a in _all_cands_scores]
        all_scores = [a[2] for a in _all_cands_scores]
        assert all_scores[0] == OPT_SCORE
        # Get representations
        all_reprs = get_representations(all_cands,
                                        repr_model=repr_model,
                                        pooling=args.repr_llm_pooling,
                                        normalize=args.normalize_inputs,
                                        prompt=args.repr_prompt,
                                        proj_matrix=proj_matrix,
                                        diff_repr=diff_repr,
                                        device=device).view(len(all_cands), -1)
        print(f"\nLoaded {len(all_cands)} oracle selection candidates\n")

        if args.visualize_posterior:
            viz_cands, viz_repr, viz_scores = all_cands, all_reprs, torch.tensor(all_scores)
    ...
    if requires_bo and args.visualize_posterior:
        ...
        for x, y in dataloader:
            posterior = surrogate.posterior(x.to(device).to(TORCH_DTYPE[args.dtype]))
            with torch.no_grad():
...
{warmstart_best_x[1]}\n""")
    ...
    for t in pbar:
        ...
        for _i in range(opt_batch_size):
            proposed_x_repr = torch.zeros(sampled_x_repr[0].shape).to(device)
            if requires_bo:
                ...
                all_sims = cosine_similarity(proposed_x_repr[None, :], all_reprs)
                ...
                all_sims = cosine_similarity(sampled_x_repr[top_seen_idxs[0]][None, :], all_reprs)
                ...
                top_seen_cands = [sampled_x[i] for i in top_seen_idxs]
                # Get similarity w.r.t. the highest scoring candidate
                all_sims = torch.randn(len(all_reprs))
            else:
                raise NotImplementedError

            # Create a filtered list of unseen cands and scores from unlabeled_cands and all_sims
            unseen_cands_scores_idxs_bbox = [(all_cands[i], all_sims[i].item(), i, all_scores[i]) for i in
                                             range(len(all_cands)) if
                                             not is_duplicate(all_cands[i], top_seen_cands)]
            target_cand_score_bbox_idx = []
            for i in range(len(unseen_cands_scores_idxs_bbox)):
                if unseen_cands_scores_idxs_bbox[i][3] == OPT_SCORE:
                    target_cand_score_bbox_idx.append(unseen_cands_scores_idxs_bbox[i])
                else:
                    break
            assert len(target_cand_score_bbox_idx) != 0
            # Sort by similarity (descending)
            unseen_cands_scores_idxs_bbox = sorted(unseen_cands_scores_idxs_bbox, key=lambda _x: _x[1], reverse=True)

            print_targets = []
            for _target_tuple in target_cand_score_bbox_idx:
                _target_tuple_rank = unseen_cands_scores_idxs_bbox.index(_target_tuple) + 1
                print_targets.append((_target_tuple_rank,
                                      f"Rank of target: {_target_tuple_rank} (similarity={round(_target_tuple[1], 6)})"))
                if best_oracle_rank_sim_iter is None or _target_tuple_rank < best_oracle_rank_sim_iter[0]:
                    best_oracle_rank_sim_iter = (_target_tuple_rank, round(_target_tuple[1], 6), t)
            # Sort print_targets by rank (ascending)
            print_targets = sorted(print_targets, key=lambda _x: _x[0])
            for _print_target in print_targets:
                print(_print_target[1])

            # Sample batch_size candidates
            if args.oracle_pick_best:
                sampled_idxs = [unseen_cands_scores_idxs_bbox[i][2] for i in range(args.vec2text_batch_size)]
            else:
                # Sample weighted by similarity
                sampled_idxs = np.random.choice(
                    [unseen_cands_scores_idxs_bbox[i][2] for i in range(len(unseen_cands_scores_idxs_bbox))],
                    size=args.vec2text_batch_size,
                    replace=False,
                    p=softmax([(unseen_cands_scores_idxs_bbox[i][1] + 1e-6) / (args.llm_temperature + 1e-6) for i in
                               range(len(unseen_cands_scores_idxs_bbox))]))
            decoded_cands = [(all_cands[i], all_reprs[i], proposed_x_repr) for i in sampled_idxs]
...
{proposed_x[1]}\n""")
            ...
            if not is_valid_proposal:
                n_invalid_bbox += 1
                if args.oracle_fix_all_cands:
                    for _cand_i, _cand in enumerate(raw_all_cands[target.split(".")[0]]):
                        if _cand["docstring"] == proposed_x[0] and _cand["code"] == proposed_x[1]:
                            _cand["score"] = INV_SCORE
                            oracle_fix_all_cands_count += 1
                            break
                if args.surr_skip_invalid_candidates:
            ...
            # if requires_bo:  # Commented for oracle setting
            sampled_x_repr = torch.cat([sampled_x_repr, proposed_x_repr.unsqueeze(0)], dim=0)
            proposed_preds = get_ladder_data(proposed_preds, expected_pred_dim, dtype=proposed_y.dtype, device=device)
            sampled_y_preds = torch.cat([sampled_y_preds, proposed_preds.view(-1, expected_pred_dim)], dim=0)
            best_idx = sampled_y.argmax().item()
        ...
        if requires_bo:
            ...
            if len(posterior_update_x) > 0 and not opt_found:
                ...
                surrogate = get_surrogate(args.surrogate_fn, sampled_x_repr, sampled_y, train_yvar=args.gp_noise_var,
                                          ...
                                          standardize_outputs=args.surr_standardize_outputs,
                                          bounds=bounds, dtype=args.dtype,
                                          device=device, **{**add_kwargs, "ladder_kernel_data": sampled_y_preds})
    pbar.close()
    time_end = time.time()
    ...
    if requires_bo and args.visualize_posterior:
        ...
        plot_posterior(posterior_vals=posterior_vals, obs_xy=viz_observed, posterior_cands=viz_cands,
                       animate=args.visualize_posterior_anim, anim_interval=300, anim_repeat=True,
                       path=posterior_path.replace('posterior', 'posterior-top'), top_k=int(len(all_cands) / 10))
    ...
    rtn = {
        ...
        "seed": seed,
        "n_oracle_cands": len(all_cands),
        "oracle_best_rank_sim_iter": best_oracle_rank_sim_iter,
    ...
    # If fix all cands, save the fixed cands
    if args.oracle_fix_all_cands and oracle_fix_all_cands_count > 0:
        with open(args.all_cands_path, "w") as fh:
            fh.write(json.dumps(raw_all_cands, indent=2))
        print(f"Fixed {oracle_fix_all_cands_count} cands in {args.all_cands_path}")
...
def run_bo(target, n_runs=1, seed=17, device='cuda'):
    ...
    overall_solved = round(sum([1 for v in all_summary.values() if v["n_opt"] == 1]) / len(all_summary), 4)
    overall_y_found = round(np.mean([v["avg_best_y"] for v in all_summary.values()]), 4)
    overall_y_test = round(np.mean([v["avg_test_score"] for v in all_summary.values()]), 4)
    print(
        f"\nTasks without errors: {len(all_summary)}, overall solved: {overall_solved}, overall y_found: {overall_y_found}, overall y_test: {overall_y_test}\n")
