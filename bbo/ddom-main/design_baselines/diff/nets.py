...
class DiffusionTest(pl.LightningModule):
    ...
    def __init__(
        ...
        self.clip_min = torch.tensor(task.x).reshape(task.x.shape[0], -1).min(axis=0)[0]
        self.clip_max = torch.tensor(task.x).reshape(task.x.shape[0], -1).max(axis=0)[0]
