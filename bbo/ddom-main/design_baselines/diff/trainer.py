...
def run_training_forward(
    ...
    if normalise_x and not task.is_discrete:
    ...
    periodic_checkpoint_callback = pl.callbacks.ModelCheckpoint(
        ...
        every_n_train_steps=checkpoint_every_n_steps if checkpoint_every_n_steps > 0 else None,
        train_time_interval=pd.Timedelta(checkpoint_time_interval).
        to_pytimedelta() if checkpoint_time_interval is not None and checkpoint_time_interval != -1 and checkpoint_time_interval != "-1" else None,
    )
    ...
    trainer = pl.Trainer(
        ...
        max_epochs=epochs,
        max_steps=max_steps,
        max_time=train_time if train_time != -1 and train_time != "-1" else None,
        logger=wandb_logger,
    ...
    # Create latest-run symlink so eval can find checkpoints (wandb offline mode)
    _wandb_dir = os.path.dirname(wandb_logger.experiment.dir)
    _latest_link = os.path.join(_wandb_dir, "latest-run")
    if os.path.islink(_latest_link) or os.path.exists(_latest_link):
        os.unlink(_latest_link)
    os.symlink(wandb_logger.experiment.dir, _latest_link)
...
def run_evaluate(
    ...
    def heun_sampler(sde, x_0, ya, num_steps, lmbd=0., keep_all_samples=True, clip_min=None, clip_max=None):
        ...
        with torch.no_grad():
            for i in range(num_steps):
                ...
                if clip_min is not None and clip_max is not None:
                    x_t = torch.clip(x_t, min=clip_min, max=clip_max)
    ...
    def _get_trained_model():
        ...
        if not os.path.exists(checkpoint_path):
            print(f"Forward model checkpoint not found at {checkpoint_path}, skipping forward model predictions")
            return None
    ...
    for lmbd in lmbds:
        ...
        xs = heun_sampler(model,
                          ...
                          clip_min=model.clip_min.cuda(),
                          clip_max=model.clip_max.cuda())  # sample
    ...
    if not designs:
        print("ERROR: No valid designs generated, all samples were NaN")
        sys.exit(1)
    ...
    print(designs.shape)
    print(results.shape)
    print(f"best score: {results.max():.6f}")
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        if not os.path.exists(checkpoint_path):
            wandb_dir = os.path.join(expt_save_path, "wandb")
            if os.path.isdir(wandb_dir):
                runs = sorted([d for d in os.listdir(wandb_dir) if d.startswith("offline-run-")])
                if runs:
                    checkpoint_path = os.path.join(wandb_dir, runs[-1], "files/checkpoints/last.ckpt")
