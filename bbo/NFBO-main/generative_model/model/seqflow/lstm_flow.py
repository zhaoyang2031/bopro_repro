...
from .utils import reverse_padded_sequence, make_pos_cond
...
class LSTM_AFLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        y, logdet, hiddens, rnn_cond_inp, lengths = inputs
        ...
        if not self.notimecontext:
            ...
            y_packed = nn.utils.rnn.pack_padded_sequence(y_packed, lengths_inp.cpu())
    ...
    def generate(self, inputs):
        ...
        y = x.clone()#torch.tensor(x) # [T, B, inp_dim]
...
class AFPrior(nn.Module):
    def __init__(self, hidden_size, zsize, dropout_p, dropout_locations, prior_type, num_flow_layers, rnn_layers, max_T=-1,
                 transform_function='affine', hiddenflow_params={}, **kwargs):
        super().__init__()
        
        self.input_embedding = torch.nn.Embedding(kwargs['vocab_size'], zsize)

        self.noise = kwargs['noise']
        self.sim_coef = kwargs['sim_coef']
        
        self.cliping = kwargs['cliping']
        self.max_T = max_T
    ...
    def forward(self, x, weight=None):
        ...
        _, z, hiddens, z_all = self.evaluate(x_emb[:,:,None,:], lengths, pos_cond)
        cos = nn.CosineSimilarity(dim=-1)
        x_noise = x_emb + torch.randn(x_emb.shape, device=device) * self.noise
        sim = cos(x_noise[...,None,:], self.input_embedding.weight)
        ...
        if self.cliping:
            ...
            for i in range(limit):
                if mask.all():
                    break
                x_noise[~mask] = x_emb[~mask] + torch.randn(x_emb[~mask].shape, device=device) * self.noise
                sim = cos(x_noise[...,None,:], self.input_embedding.weight)
                mask = (sim.argmax(-1)==x)
    
        K, F = self.input_embedding.weight.shape
        label = nn.functional.one_hot(x, K)
        negative = torch.zeros(T,B,K, device=device).scatter_(2, torch.randint(K, (T,B, 1), device=device), -1)
        label = torch.where(label==1, label, negative)
        crieterion = nn.CosineEmbeddingLoss(reduction='none')
        recon_loss = crieterion(x_noise.repeat(1,1,K,1).reshape(-1, F), self.input_embedding.weight[None,None,...].repeat(T,B,1,1).reshape(-1, F), label.reshape(-1)).reshape(T,B,-1)
        recon_loss = recon_loss.sum(-1)[...,None]
        
        x_noise = x_noise[:,:,None,:]
        log_p_z, _, _, _ = self.evaluate(x_noise, lengths, pos_cond)
        log_p_loss = -log_p_z
        if self.cliping:
            total_loss = log_p_loss + recon_loss * self.sim_coef
            total_loss = total_loss.mean(-1).sum(0).mean(0)
            log_p_loss = log_p_loss.mean()
        ...
        out_dict = {"log_p_loss": log_p_loss}
        if self.cliping:
            out_dict.update({"recon_loss": recon_loss})
        else:
            out_dict.update({"recon_loss": recon_loss})
        return z[:,:,0,:].permute(1,0,2), total_loss, out_dict
    ...
    def encode_z(self, x, sampling=True, lengths=None):
        ...
        x_emb = self.input_embedding(x)
        x_emb = torch.nn.functional.normalize(x_emb, p=2, dim=-1)
        x_emb = x_emb.permute(1,0,2)[:,:,None,:]
        log_p_z, z, hiddens, z_all = self.evaluate(x_emb, lengths, pos_cond)
        return z[:,:,0,:].permute(1,0,2), log_p_z, None
    ...
    def decode_z(self, z=None, sampling=False, length=40, B=None, temp=20):
        ...
        pos_cond = make_pos_cond(T, B, lengths, self.max_T)
        x, log_p_zs, z_, hidden = self.generate(lengths, pos_cond, eps=z)
        x = x.permute(1,0,2)
        cos = nn.CosineSimilarity(dim=-1)
        sim = cos(x[...,None,:], self.input_embedding.weight)
        sim = torch.where(torch.isnan(sim), 1, sim)
        if sampling == False:
            sample = sim.argmax(-1)
        else:
            probs = torch.softmax(sim*temp, dim=-1)
            num_samples = 1
            sample = torch.multinomial(probs.reshape(B*T, -1), num_samples, replacement=True).reshape(B,T,num_samples).permute(0,2,1).reshape(B*num_samples, T)
            # sample = sim.argmax(-1)
        ...
        if sampling == False:
            return sample, valid_mask, log_p_zs
        else:
            return sample, valid_mask, log_p_zs, probs
    def evaluate(self, z, lengths_s, cond_inp_s=None):
        ...
        eps, logdet, hiddens, z_all = self.flow(z, hidden, lengths_s, rnn_cond_inp=cond_inp_s)
        ...
        return log_p_z, eps, hiddens, z_all

    def generate(self, lengths, cond_inp=None, eps=None, temp=1.0):
        T = torch.max(lengths)
        B = lengths.shape[0]
