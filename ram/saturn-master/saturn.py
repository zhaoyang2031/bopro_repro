...
if __name__ == "__main__":
    ...
    if running_mode == "distribution_learning":
        ...
        _task = _os.path.basename(args.config).replace('.json', '')
        wandb.init(project='repro_ram', entity=_os.environ.get('WANDB_ENTITY', '1585515136-'), name=f'Saturn_{_task}', config=config, reinit=True)
        ...
        # Load offline data if specified (full-offline mode)
        offline_dataset = None
        if "offline_data_path" in config["goal_directed_generation"]:
            import torch
            offline_path = config["goal_directed_generation"]["offline_data_path"]
            offline_dataset = torch.load(offline_path, weights_only=False)
            print(f"Loaded offline data: {len(offline_dataset)} molecules")
