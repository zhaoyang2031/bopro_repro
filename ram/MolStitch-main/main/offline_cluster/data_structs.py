...
class Experience(object):
    ...
    def __init__(self, voc: Vocabulary, max_size=100):
        self.memory = []
        self.pareto_memory = []
    ...
    def add_pareto_experience(self, experience, candidates):
        """Adopted from DyMol"""
    ...
    def sample(self, n, weighted=True):
        ...
        if len(self.memory)<n:
            ...
            scores = [np.sum((x[1]+1e-10) * self.final_weight, axis=-1) for x in self.memory]  # final + weight 변경?
            if weighted:
    ...
    def sample_smiles(self, n, weighted=True):
        """Sample a batch size n of experience"""
        if len(self.memory)<n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
        ...
        # encoded = [Variable(self.voc.encode(tokenized_i)) for tokenized_i in tokenized]
        # encoded = MolData.collate_fn(encoded)
        return smiles, np.array(scores), np.array(prior_likelihood)

    def sample_pref(self, n, pref: np.array, score_pref: np.array=None, weighted=True):
        """
        experience replay with certain preferences
        :param n: number of sample
        :param pref: preference weight of objective for each sampling
        :param score_pref: weight for scalarization
        :param weighted: False for random sampling
        :return:
        """

        if score_pref is None:
            score_pref = self.final_weight
        if len(self.memory)<n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
            scores = [np.sum((x[1]+1e-10) * pref, axis=-1) for idx, x in enumerate(self.memory)]  # final + weight 변경?
            if weighted:
                sample = np.random.choice(len(self), size=n, replace=False, p=scores/np.sum(scores))
            else:
                sample = np.random.choice(len(self), size=n, replace=False)
            sample = [self.memory[i] for i in sample]
            smiles = [x[0] for x in sample]
            scores = [np.sum(x[1] * score_pref, axis=-1) for x in sample]
            prior_likelihood = [x[2] for x in sample]
        tokenized = [self.voc.tokenize(smile) for smile in smiles]
        encoded = [Variable(self.voc.encode(tokenized_i)) for tokenized_i in tokenized]
        encoded = MolData.collate_fn(encoded)
    ...
    def rank_based_sample(self, n, rank_coefficient=0.01):
        """Adopted from GeneticGFN"""
        if len(self.memory)<n:
            raise IndexError('Size of memory ({}) is less than requested sample ({})'.format(len(self), n))
        else:
            scores_np = np.array([np.sum((x[1]+1e-10) * self.final_weight, axis=-1) for x in self.memory])  # final + weight 변경?
            ...
            for i in indices:
                try:
                    ...
                    valid_scores.append(scores_np[i])
                except:
                    pass
