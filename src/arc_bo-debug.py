...
def _bo(target, gen_model, repr_model, bbox_model, prompt_cls, task=None,
    ...
    if requires_bo:
        ...
        learned_gp_mean = surrogate.mean_module.constant.item()
        ...
        for _i in range(opt_batch_size):
            ...
            if all_cands is None:
                ...
                if requires_bo:
                    ...
                    top_seen_idxs = seen_sims.argsort(descending=True)[:args.vec2text_demos].tolist()
                    ...
                    # Implement true greedy
                    # Pick batch_size candidates that are most similar to the best vec2text_demos seen candidates
                    top_seen_idxs = sampled_y.argsort(descending=True)[:args.vec2text_demos].tolist()
                    top_seen_cands = [sampled_x[i] for i in top_seen_idxs]
                    top_seen_reprs = sampled_x_repr[top_seen_idxs]
                    # Find the most similar k candidates in all_cands to the top_seen_reprs
                    _all_sims = []
                    for _repr in top_seen_reprs:
                        _all_sims.append(cosine_similarity(_repr[None, :], all_reprs))
                    all_sims = torch.stack(_all_sims).mean(dim=0)
                else:
                    raise NotImplementedError

                top_sim_idxs = all_sims.argsort(descending=True)[
                               :args.vec2text_batch_size * 2 + args.vec2text_demos].tolist()
                # Filter out seen candidates
                top_sim_idxs = [i for i in top_sim_idxs if not is_duplicate(all_cands[i], top_seen_cands)]
                # Sample batch_size candidates from the remaining candidates weighted by similarity
                sampled_idxs = np.random.choice(top_sim_idxs,
                                                size=args.vec2text_batch_size,
                                                replace=False,
                                                p=softmax(all_sims[top_sim_idxs].cpu().numpy()))
                decoded_cands = [(all_cands[i], all_reprs[i]) for i in sampled_idxs]
                # decoded_cands = [(all_cands[i], all_reprs[i]) for i in top_sim_idxs \
                #          if (args.repeat_cand_strategy != "skip" or not is_duplicate(all_cands[i], sampled_x))]
...
{proposed_x[1]}\n""")
    ...
    rtn = {
        ...
        "learned_surrogate_mean": round(learned_gp_mean, 4) if learned_gp_mean is not None else None,
