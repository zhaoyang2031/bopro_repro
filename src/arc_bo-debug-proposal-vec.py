...
from utils.generation import HF_PIPELINE_MODELS, BEDROCK_CLAUDE_MODELS, SYS_PROMPT, \
    ...
    BEDROCK_COHERE_MODELS, invoke_bedrock_embeddings, HF_SFORMER_MODELS, fix_candidate
    ...
    for decoded_x in decoded_xs:
        ...
        if is_duplicate(decoded_x, sampled_x):
            if handle_duplicates:
                        ...
                        if args.repeat_streak_perturb:
                            ...
                            for repeat_streak_retry_idx in range(3):
                                ...
                                if decoded_cands is not None:
                                    decoded_x, decoded_x_repr = decoded_cands[0]
                ...
                return None
        rtn.append((decoded_x, decoded_x_repr))
...
def _bo(target, gen_model, repr_model, bbox_model, prompt_cls, task=None,
    ...
    # DEBUG CODE:
    all_cands, all_reprs = None, None
    if not opt_found and args.all_cands_path is not None:
        with open(args.all_cands_path, "r") as fh:
            all_cands = [(a['docstring'], a['code']) for a in json.load(fh)[target.split(".")[0]]]
            # Get representations
...
{warmstart_best_x[1]}\n""")
    ...
    for t in pbar:
        ...
        posterior_update_x, posterior_update_y, posterior_update_noise = [], [], []
        ...
        for _i in range(opt_batch_size):
            ...
            if requires_bo:
                ...
                bo_proposals.append(proposed_x_repr)
            ...
            def _get_rand_cands():
                random_x, _ = get_candidates(n_cands=args.vec2text_batch_size,
                                             batch_size=args.candidate_gen_batch_size,
                                             gen_model=gen_model, prompt_cls=prompt_cls,
                                             strategy="random", n_parallel=args.candidate_gen_n_parallel,
                                             task_demos=task["train"][:args.bbox_n_train],
                                             codegen=args.bbox_model == "codegen",
                                             ...
                                             transpose=args.arc_show_transpose,
                                             priors=args.arc_use_priors)
                random_reprs = get_representations(random_x,
                                                   repr_model=repr_model,
                                                   pooling=args.repr_llm_pooling,
                                                   normalize=args.normalize_inputs,
                                                   prompt=args.repr_prompt,
                                                   ...
                                                   diff_repr=diff_repr,
                                                   device=device)
                # Add each (random_x, random_reprs) to random_cands if random_x is unique
                for __i in range(len(random_x)):
                    if not is_duplicate(random_x[__i], [c[0] for c in random_cands]):
                        random_cands.append((random_x[__i], random_reprs[__i]))
                return None

            decoded_cands = None
            if all_cands is None:
                with ThreadPoolExecutor(max_workers=2) as executor:
                    futures = [executor.submit(lambda: decode_repr(proposed_x_repr=proposed_x_repr, sampled_x=sampled_x,
                                                                   sampled_x_repr=sampled_x_repr, prompt_cls=prompt_cls,
                                                                   n_cands=args.vec2text_batch_size,
                                                                   task_demos=task["train"][:args.bbox_n_train],
                                                                   ...
                                                                   counters=_counters_for_decoding)),
                               executor.submit(lambda: _get_rand_cands()) if requires_bo else None]
                    for future in as_completed([f for f in futures if f is not None]):
                        try:
                            _res = future.result()
                            if _res is not None:
                                decoded_cands = _res
                ...
                # Unpack counters
                repeat_streak, n_repeat_streak_reached, n_repeats_solved_by_perturbing, n_throttling_errors = \
                    (_counters_for_decoding["repeat_streak"], _counters_for_decoding["n_repeat_streak_reached"],
                     _counters_for_decoding["n_repeats_solved_by_perturbing"],
                     _counters_for_decoding["n_throttling_errors"])
                ...
                if requires_bo:
                    if decoded_cands is None:
                        ...
                        decoded_sims = cosine_similarity(proposed_x_repr[None, :],
                                                         get_representations([d[0] for d in decoded_cands],
                                                                             repr_model=repr_model,
                                                                             pooling=args.repr_llm_pooling,
                                                                             normalize=args.normalize_inputs,
                                                                             prompt=args.repr_prompt,
                    ...
                    random_sims = cosine_similarity(proposed_x_repr[None, :],
                                                    torch.stack([r[1] for r in random_cands])).tolist()
                    # Pick top-vec2text_batch_size candidates
                    top_sim_idxs = torch.tensor(decoded_sims + random_sims).argsort(descending=True)[
                                   :args.vec2text_batch_size].tolist()
                    _n_rand_picked = sum(idx >= len(decoded_cands) for idx in top_sim_idxs)
                    n_rand_picked += _n_rand_picked
                    dec_and_rand = decoded_cands + random_cands
                    decoded_cands = [dec_and_rand[i] for i in top_sim_idxs if (
                            args.repeat_cand_strategy != "skip" or not is_duplicate(dec_and_rand[i][0], sampled_x))]
                    # Handle duplicates in the selected random candidates
                    if args.repeat_cand_strategy == "bo_proposal":
                        for _i, d in enumerate(decoded_cands):
                            if is_duplicate(d[0], sampled_x):
                                decoded_cands[_i] = (decoded_cands[_i][0], proposed_x_repr)

                    # Remove picked rand candidates from random_cands
                    random_cands = [r for i, r in enumerate(random_cands) if i + len(decoded_cands) not in top_sim_idxs]
                    assert len(decoded_cands) == args.vec2text_batch_size
                    print(f"Random candidates picked: {_n_rand_picked}/{len(random_cands) + _n_rand_picked}")
                    print(f"Random candidates left: {len(random_cands)}")
            else:
                assert requires_bo
                # Find the most similar k candidates in all_cands
                all_sims = cosine_similarity(proposed_x_repr[None, :], all_reprs)
                top_sim_idxs = all_sims.argsort(descending=True)[:args.vec2text_batch_size].tolist()
                decoded_cands = [(all_cands[i], all_reprs[i]) for i in top_sim_idxs if (
                        args.repeat_cand_strategy != "skip" or not is_duplicate(all_cands[i], sampled_x))]
                # Handle duplicates in the selected random candidates
                if args.repeat_cand_strategy == "bo_proposal":
                    for _i, d in enumerate(decoded_cands):
                        if is_duplicate(d[0], sampled_x):
                            decoded_cands[_i] = (decoded_cands[_i][0], proposed_x_repr)
                # # Remove picked candidates from all_cands
                # all_cands = [c for i, c in enumerate(all_cands) if i not in top_sim_idxs]
                # all_reprs = [r for i, r in enumerate(all_reprs) if i not in top_sim_idxs]
                # all_reprs = torch.stack(all_reprs) if len(all_reprs) > 0 else None
            ...
            scores_from_this_proposal_vector = []
            for decoded_cand in decoded_cands:
                proposed_x, proposed_x_repr = decoded_cand
...
{proposed_x[1]}\n""")
                ...
                if requires_bo:
                    scores_from_this_proposal_vector.append(proposed_y.item())
                    # posterior_update_x.append(proposed_x_repr.view(1, -1))
                    # posterior_update_y.append(proposed_y.view(-1))
                    # posterior_update_noise.append(torch.tensor([args.gp_noise_var + 1e-6]).view(-1, 1))
            ...
            if requires_bo:
                mean_score = np.mean(scores_from_this_proposal_vector)
                var_score = np.var(scores_from_this_proposal_vector)
                posterior_update_x.append(bo_proposals[-1].view(1, -1))
                posterior_update_y.append(torch.tensor(mean_score).reshape(proposed_y.shape).view(-1).to(proposed_y.device))
                posterior_update_noise.append(torch.tensor([var_score]).view(-1, 1))
        ...
        if requires_bo:
            ...
            if len(posterior_update_x) > 0 and not opt_found:
                ...
                posterior_update_noise = torch.cat(posterior_update_noise, dim=0).view(-1, 1)

                print(f"posterior_update_x:\n{posterior_update_x}")
                print(f"posterior_update_y:\n{posterior_update_y}")
                print(f"posterior_update_noise:\n{posterior_update_noise}")
                ...
                surrogate = surrogate.condition_on_observations(
                    ...
                    noise=posterior_update_noise.to(device).to(TORCH_DTYPE[args.dtype]),
                )
    pbar.close()
    time_end = time.time()
    ...
    if requires_bo and args.visualize_posterior and (not opt_found or steps_to_opt > 0):
        os.makedirs(out_dir, exist_ok=True)
...
def run_bo(target, n_runs=1, seed=17, device='cuda'):
    ...
    if args.use_method_defaults:
        ...
        if args.acquisition_fn in NON_BO_ACQS:
            ...
            args.vec2text_normalize = args.vec2text_exploit_threshold = False
            print("Force set vec2text_normalize, vec2text_target_score, vec2text_exploit_threshold to False")
            if args.acquisition_fn == "none":
                args.arc_no_demos = True
                print("Force set arc_no_demos to True")
        else:
            # BO
            args.vec2text_target_score = args.vec2text_normalize = True
